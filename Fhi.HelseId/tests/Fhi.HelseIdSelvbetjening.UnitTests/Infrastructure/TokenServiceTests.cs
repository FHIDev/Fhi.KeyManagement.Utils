    ...
    public class TokenServiceTests
    {
        ...
        public async Task GetDPoPToken_InvalidAuthority_DiscoveryendpointLookupReturnsError()
        {
            ...
            using (Assert.EnterMultipleScope())
            {
                ...
                Assert.That(tokenResponse.ErrorDescription, Is.EqualTo("Error connecting to https://wronguri/.well-known/openid-configuration: Bad Request"));
                Assert.That(tokenResponse.AccessToken, Is.Null);
            }
        }

        [Test]
        public void GetDPoPToken_TokenRequestError_ErrorIsTrue()
        {
        }

        [Test]
        public void GetDPoPToken_TokenRequestSuccess_ErrorIsFalse()
        {
        }
